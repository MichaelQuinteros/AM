...
Original file is located at
    https://colab.research.google.com/drive/1T8qvmoxcfUix2WOM6_q6rzLDzyQ1fOCq
"""
...
def convert_to_pdf(input_docx, output_pdf):
    ...
    if system_os == "Windows":
        ...
        subprocess.run(["libreoffice", "--headless", "--convert-to", "pdf", input_docx])
...
def generate_final_docx(output_docx, output_pdf, plantilla_docx, distrito, provincia, departamento, tables_by_file):
    ...
    first_file = list(tables_by_file.keys())[0]
    table = tables_by_file[first_file]

    selected_columns = ['PobrezaMonetaria', 'CCPP', 'Poblacion', 'Hogares', 'Viviendas', 'UnidadTerritorial']
    data_dict = table[selected_columns].iloc[0].to_dict() if not table.empty else {col: 'N/A' for col in selected_columns}
    ...
    if "datos3.xlsx" in tables_by_file and not tables_by_file["datos3.xlsx"].empty:
        seleccionado = "SI" if any(tables_by_file["datos3.xlsx"]['Distrito'].str.lower() == distrito.lower()) else "NO"

    if "datos4.xlsx" in tables_by_file and not tables_by_file["datos4.xlsx"].empty:
        mejorando = "SI" if any(tables_by_file["datos4.xlsx"]['Distrito'].str.lower() == distrito.lower()) else "NO"

    if "datos5.xlsx" in tables_by_file:
        table_soc = tables_by_file["datos5.xlsx"]
        if not table_soc.empty:
            social = "SI"
            infraestructura_social_df = table_soc[['Provincia', 'Distrito', 'CentroPoblado']] if all(col in table_soc.columns for col in ['Provincia', 'Distrito', 'CentroPoblado']) else pd.DataFrame()
...
output_docx = f"informe_{distrito}.docx"
output_pdf = f"informe_{distrito}.pdf"
